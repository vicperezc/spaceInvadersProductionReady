...
Invaders.MechanicFour = function () {
    ...
    //playerHitsInvadersExtra Variables
    this.deadBoxX = [];
    this.deadBoxY = [];
    this.rN = 0;
...
Invaders.MechanicFour.prototype = {
    ...
    create: function () {
        ...
        this.invaderBulletDos.fireRate = 800;
    ...
    playerHitsInvadersExtra: function (enemy,bullet) {
        ...
        for (var i = 0; i < 60; i++){
            ...
            naves.push(this.invadersGroup.children[i]);
            campos.push(this.campoDeFuerzaGroup.children[i]);
        }
        var invaderVivos = [];
        var camposVivos = [];
        for (i = 0; i < naves.length; i++) {
            ...
            if (naves[i].alive ){
                // Si están vivos los zombies los metemos al array zombiesAlive []
                invaderVivos.push(naves[i]);
                camposVivos.push(campos[i]);
            }
        }

        var invaderFall = null;
        var campoFall = null;

        this.deadBoxX.push(enemy.x);
        this.deadBoxY.push(enemy.y);


        this.rN = Math.floor(Math.random()*(this.deadBoxX.length)); //  Math.floor(Math.random()*(max-min+1)+min);

        var arr24 = [];
        while(arr24.length < 1){
            var randomnumber2=Math.floor(Math.random()*(invaderVivos.length)); //  Math.floor(Math.random()*(max-min+1)+min); //  Math.floor(Math.random()*(max-min+1)+min);
            var found2=false;
        ...
        if (invaderVivos.length == 59){

            if (invaderVivos.length > 1) {
                // Posicion de group
                //invaderFall = this.add.tween(singleInvader);
                //invaderFall = this.add.tween(invaderVivosRandom);
                invaderFall = this.add.tween(invaderVivos[arr24]);
                invaderFall.to({
                    x: this.deadBoxX[0],
                    y: this.deadBoxY[0]
                ...
                campoFall = this.add.tween(camposVivos[arr24]);
                campoFall.to({
                    x: this.deadBoxX[0],
                    y: this.deadBoxY[0]
                }, 200, Phaser.Easing.Linear.None, false);
                ...
                this.deadBoxX.shift(); //Se borra el primer indice del array
                this.deadBoxY.shift();
                this.deadBoxX.push(invaderVivos[arr24].x); // meto la posición del invaderVivoRandom que llegò push invader vivo random
                this.deadBoxY.push(invaderVivos[arr24].y);
            }


        } else if (invaderVivos.length < 59){

            if (invaderVivos.length > 0){
                // Posicion de group
                //invaderFall = this.add.tween(singleInvader);
                invaderFall = this.add.tween(invaderVivos[arr24]);
                //invaderFall = this.add.tween(this.item);
                invaderFall.to({
                    x: this.deadBoxX[this.rN],
                    y: this.deadBoxY[this.rN]}, 200, Phaser.Easing.Linear.None, false);
                invaderFall.start();

                campoFall = this.add.tween(camposVivos[arr24]);
                campoFall.to({
                    x: this.deadBoxX[this.rN],
                    y: this.deadBoxY[this.rN]
                }, 200, Phaser.Easing.Linear.None, false);
                campoFall.start();
                // Esa posicion random que el invader acaba de escojer la sacamos de la dead box

                this.deadBoxX.indexOf(this.rN);
                this.deadBoxY.indexOf(this.rN);
                this.deadBoxX.splice(this.rN,1);
                this.deadBoxY.splice(this.rN,1);
    ...
    update: function () {
        ...
        this.physics.arcade.overlap(this.invadersGroup, this.bulletRed.bullets,this.playerHitsInvadersExtra,null,this);
    ...
    shutdown: function () {
        //Para reiniciar valores cada vez que cambiamos de state
        this.ufoS.stop();
        this.youWinS.stop();
    ...
    render: function () {
        ...
        this.game.debug.text('M4 Counter: ' + this.counter,20,80);
